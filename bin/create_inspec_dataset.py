...
def _generate_summary(client: OpenAI, docs: List[str], keyword: str) -> str:
    ...
    with open(".dump/debug.log.jsonl", "a") as f:
        debug = { "keyword": keyword, "input": joined_docs, "output": response, "is_error": len(response.split(".")) > 2 if response else True} 
...
def main(base_path: str = ".dump/Inspec", sample_size: int = 3, seed: int = 42):
    ...
    summaries_df = pd.DataFrame(keywords_descriptions.items(), columns=["keyword", "description"])
