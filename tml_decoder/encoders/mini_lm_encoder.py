...
class MiniLMEncoder(AbstractEncoder):
    ...
    def get_unused_token(self) -> str:
        return next(k for k, v in self.encoder.tokenizer.vocab.items() if "[unused" in k)
    ...
    def unfreeze_embedding_weights(self):
        for idx, p in enumerate(self.encoder[0].auto_model.embeddings.word_embeddings.parameters()):
